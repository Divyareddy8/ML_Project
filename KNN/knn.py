...
knn_grid = GridSearchCV(knn_pipe, param_grid_knn, cv=cv, scoring='neg_root_mean_squared_error', n_jobs=-1)
...
knn_best = Pipeline([
    ...
    ('regressor', KNeighborsRegressor(
        n_neighbors=best_knn_params['regressor__n_neighbors'],
        weights=best_knn_params['regressor__weights'],
        p=best_knn_params['regressor__p'],
        n_jobs=-1
    ))
])
knn_best.fit(X, y)
...
mse_knn, rmse_knn, r2_knn = evaluate_and_save(
    "KNN Regressor (Tuned)", knn_best, X_val, y_val, test.drop('Hospital_Id', axis=1), "KNN_submission_tuned.csv"
)
